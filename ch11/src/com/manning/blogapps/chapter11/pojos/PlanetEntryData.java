 ...
 */
package com.manning.blogapps.chapter11.pojos;
...
public class PlanetEntryData implements Serializable, Comparable {
    ...
    private long   enclosureLength = 0;
    ...
    private void initFromRomeEntry(SyndFeed romeFeed, SyndEntry romeEntry) {
        ...
        // get first enclosure only (good enough for RSS 2.0 podcasts)
        if (romeEntry.getEnclosures() != null && romeEntry.getEnclosures().size() > 0) {
            SyndEnclosure enc = (SyndEnclosure)romeEntry.getEnclosures().get(0);
            setEnclosureURL(enc.getUrl());
            setEnclosureType(enc.getType());
            setEnclosureLength(enc.getLength());
        }
    }
